from app.game.ships import (
    ...
    parse_coordinate, coordinate_to_string
)
from app.game.state import GameState, BombResult
from app.game.board import (
    render_all_public_boards, render_private_board, boards_to_bytes
)
from app.database import EventType, Role
from app.models import (
    get_player_by_chat, get_player_by_id, get_player_by_color,
    create_player, get_all_teams, get_all_game_masters,
    add_event, get_all_events, get_location_by_number,
    get_next_location_number, create_location, get_all_locations
)
...
async def handle_join(db, update: Update, context: ContextTypes.DEFAULT_TYPE, team_name: str):
    ...
    await add_event(db, EventType.TEAM_JOINED, {
        "name": team_name,
        "color": color,
        "chat_id": chat_id,
        "bombs": 3
...
async def handle_place(db, update: Update, context: ContextTypes.DEFAULT_TYPE, ship_type: str, coord: str, direction: str):
    chat_id = update.effective_chat.id
    ...
    await add_event(db, EventType.SHIP_PLACED, {
        "color": player.color,
        "ship_type": ship_type,
        "row": row,
        "col": col,
    ...
    await send_photo(context, chat_id, img_bytes, caption=f"Ship placed! Ships left: {SHIP_COUNTS[ship_type] - team.placed_ship_types.get(ship_type, 0)}")
    
    return None


async def handle_bomb(db, update: Update, context: ContextTypes.DEFAULT_TYPE, target_color: str, coord: str):
    chat_id = update.effective_chat.id
    ...
    await add_event(db, EventType.BOMB_THROWN, {
    ...
    if target_player:
        ...
        hit_msg = f"💥 HIT! {attacker.name} ({attacker.color}) bombed you at {coord_str}!"
...
async def handle_code(db, update: Update, context: ContextTypes.DEFAULT_TYPE, location_number: int, code: str):
    chat_id = update.effective_chat.id
    ...
    await add_event(db, EventType.CODE_REDEEMED, {
        "color": player.color,
...
async def handle_location(db, update: Update, context: ContextTypes.DEFAULT_TYPE, latitude: float, longitude: float, code: Optional[str]):
    chat_id = update.effective_chat.id
    ...
    await add_event(db, EventType.LOCATION_ADDED, {
        "number": number,
        "latitude": latitude,
        "longitude": longitude,
        "code": location_code
...
async def handle_locations_list(db, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for loc in locations:
        msg += f"{loc.number}. https://maps.google.com/?q={loc.latitude},{loc.longitude}\n"
